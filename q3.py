...
class  NaiveBayes:
	...
	def _calc_class_prior(self):
		...
		for outcome in np.unique(self.y_train):
			...
			k = np.sum(self.y_train == outcome)/total
			#TODO_1
			self.class_priors[outcome] = k #TODO_2    Remove 0 and write your code 
			### YOUR CODE ENDS HERE ###
	...
	def predict(self, X):
		...
		for query in X:
			...
			for outcome in np.unique(self.y_train):
				...
				prior = self.class_priors[outcome] #TODO_1   Take value from task-1 variable  (Remove 'None')
				likelihood = 1
				evidence = 1
				...
				posterior = (prior*likelihood)/evidence #TODO_2         Remove 0 and get posterior
                
				probs_outcome[outcome] = posterior

			result = max(probs_outcome, key=probs_outcome.get) #TODO_3          Remove None and get the Max
