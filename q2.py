...
class MultiClassLogisticRegression:
    ...
    def softmax(self, z):
        ...
        # print(z.shape)
        post_softmax = np.exp(z)
        row_sums = post_softmax.sum(axis=1, keepdims=True)
        post_softmax = post_softmax / row_sums
        ### YOUR CODE ENDS HERE ###
        return post_softmax
    ...
    def cross_entropy(self, y, probs):
        ...
        for i in range(m):
            for k in range(K):
                cost +=y[i][k]*np.log(probs[i][k])
        post_cross_entropy = -cost/m
        ### YOUR CODE ENDS HERE ###
